...
import { getHackerNewsPending, getHackerNewsPosts, getHackerNewsError } from '../store';

class HackerNewsProgressIcon extends Component {
  render () {
    const { pending, posts, error } = this.props;

    // default
    let opactiyValue = 'hacker-news-loading-rendered';

    if (pending) {
      opactiyValue = 'hacker-news-loading-pending';
    } else if (posts.length > 0) {
      opactiyValue = 'hacker-news-loading-success';
    } else if (error) {
      opactiyValue = 'hacker-news-loading-error';
    }

    return (
      <div className={opactiyValue}>
...
const mapStateToProps = (state) => ({
  pending: getHackerNewsPending(state),
  posts: getHackerNewsPosts(state),
  error: getHackerNewsError(state)
});

export default connect(mapStateToProps)(HackerNewsProgressIcon);
